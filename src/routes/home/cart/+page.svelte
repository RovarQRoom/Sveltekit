<script lang="ts">
    ...
    $: {
		if ($cartItemWritable) {
			cartItems = $cartItemWritable;
            total = cartItems.reduce((acc, item) => acc + item.total_price, 0);
		}
	}

	async function getData() {
		await cartsHandlers.getCartItems() ;
...
>
	<div class="flex items-center justify-between mb-4">
		<div class="flex-shrink-0">
			<img
				class="w-8 h-8"
				src="https://cdn-icons-png.flaticon.com/512/4219/4219909.png"
				alt="Neil image"
			/>
		</div>
				...
				<li class="py-3 sm:py-4">
					<div class="flex items-center space-x-4">
						...
						>
							...
							<Trash
								size="20"
								class="text-red-500 cursor-pointer hover:text-red-900 transition-all"
								on:click={() => deleteCartItem(index)}
							/>
						</div>
					</div>
				</li>
